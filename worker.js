/**
 * Cloudflare VLESS Worker
 * 协议: VLESS + WebSocket + TLS
 * 兼容客户端: v2rayN / NekoBox / Clash.Meta / Sing-Box / Shadowrocket
 *
 * 部署方式见 README.md（支持 Cloudflare Workers 与 Pages Functions）
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (!isValidUUID(userID)) {
      return new Response(
        'UUID 未配置。请在 wrangler.toml / Pages 环境变量中设置 UUID（或用 `wrangler secret put UUID`）。',
...
async function handleOneUDP(writer, payload, version) {
  // VLESS UDP 包: ATYPE(1) | ADDR | PORT(2) | LENGTH(2) | DATA(LENGTH)
  let offset = 0;
...
function parseVLESSRequest(data, userID) {
  ...
  // 兼容两种客户端布局:
  //  - 标准 v2ray-core: addons@17, optLen@18, cmd@19+optLen
  //  - 变体(edgetunnel 系): optLen@17, cmd@18+optLen
...
async function buildSubscription(request, env, userID, url) {
  ...
  if (isClash || isSb) {
    // 引导到订阅转换服务，直接生成 Clash / Sing-Box 配置
